...
return {
    ...
    recompute_movement = function()
        ...
        if state.mouse_controls then
            state.movement_vector.x = state.mouse.dx
            state.movement_vector.y = state.mouse.dy
            state.mouse.dx, state.mouse.dy = 0, 0
        else
            ...
            state.movement_multiplier = state.keys_down.focus and 0.3 or 1.0
